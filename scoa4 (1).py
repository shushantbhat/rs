...
for g in range(GEN):
    ...
    best_idx = np.argsort(fit)[-2:]  # top 2 parents
    p1, p2 = pop[best_idx[0]], pop[best_idx[1]]

    new_pop = []
    for _ in range(POP):
        child = [p1[0], p2[1]] if rd.random() < 0.5 else [p2[0], p1[1]]
        new_pop.append(mutate(child))

    pop = new_pop
    print(f"Generation {g+1} --> Best fitness: {max(fit):.4f}")
